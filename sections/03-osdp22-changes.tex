...
The reply continues to use the OSDP~2.2 multi-part format. Successful execution yields a response containing the credential’s entire \texttt{GENERAL AUTHENTICATE} data (minus the ISO/IEC~7816 status word), including the returned Dynamic Authentication Template; PDs \textbf{SHALL NOT} strip outer BER-TLV tags or lengths. If the credential reports an error, the PD \textbf{SHALL} return \texttt{osdp\_PDERROR} with the applicable PIV code—for example \texttt{0x1020} (function not supported by credential), \texttt{0x1021} (insufficient PD resources), \texttt{0x1022} (insufficient credential storage), \texttt{0x1023} (credential security status not satisfied), \texttt{0x1024} (data object/tag not found), \texttt{0x1025} (VCI not established), or \texttt{0x1027} (credential not present/removed). Generic parameter/security preconditions may continue to use \texttt{0x0005} (incorrect parameter) and \texttt{0x0006}/\texttt{0x0007} (PIN verification failed/timeout) where those codes remain canonical.
...
\begin{itemize}
  ...
  \item The data offset now supports one- or two-byte encodings. PDs \textbf{SHALL} accept either form and determine which was used based on the command length. The offset applies to the selected payload (entire object when the tag is zero, or the chosen tag otherwise). PDs \textbf{SHALL} return \texttt{osdp\_PDERROR} with error code \texttt{0x1023} when security prerequisites are not satisfied and \texttt{0x1024} when the requested object or tag is absent; see Subsection~\ref{sec:new-piv-error-codes}. If the credential is not present or is removed during processing, PDs may return error code \texttt{0x1027}.
\end{itemize}
...
\begin{center}
  ...
  \begin{tabular}{|c|p{9cm}|}
    ...
    0x1020 & Function not supported by credential (or PD does not implement the command). \\\hline
    ...
    0x1026 & PIV Auto not configured. \\\hline
    0x1027 & Credential not present or removed. \\\hline
    0x1028 & Biometric type not supported. \\\hline
  \end{tabular}
\end{center}
\end{samepage}
...
PDs \textbf{SHALL} use \texttt{osdp\_PDERROR} to report application-level errors. The error code is two bytes, encoded least-significant byte first. When a card/APDU failure occurs (for example SC\_ERROR or a PIV APDU failure), the PD \textbf{SHALL} append the ISO/IEC~7816 status word (SW1\textbar SW2). A single error is reported per \texttt{osdp\_PDERROR} reply. Transport-level faults (framing, CRC, sequence) continue to use \texttt{osdp\_NAK}.
