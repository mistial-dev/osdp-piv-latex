...
The reply continues to use the OSDP~2.2 multi-part format. Successful execution yields a response containing the credential’s entire \texttt{GENERAL AUTHENTICATE} data (minus the ISO/IEC~7816 status word), including the returned Dynamic Authentication Template; PDs \textbf{SHALL NOT} strip outer BER-TLV tags or lengths. If the credential reports an error, the PD \textbf{SHALL} return \texttt{osdp\_PIVERRORR} with the applicable code—for example \texttt{0x20} (function not supported by credential), \texttt{0x21} (insufficient PD resources), \texttt{0x22} (insufficient credential storage), \texttt{0x23} (credential security status not satisfied), \texttt{0x24} (data object/tag not found), \texttt{0x25} (VCI not established), or \texttt{0x27} (credential not present/removed). Generic parameter/security preconditions may continue to use \texttt{0x05} (incorrect parameter) and \texttt{0x06}/\texttt{0x07} (PIN verification failed/timeout) where those codes remain canonical.
...
\begin{itemize}
  ...
  \item The data offset now supports one- or two-byte encodings. PDs \textbf{SHALL} accept either form and determine which was used based on the command length. The offset applies to the selected payload (entire object when the tag is zero, or the chosen tag otherwise). PDs \textbf{SHALL} return \texttt{osdp\_PIVERRORR} with error code \texttt{0x23} when security prerequisites are not satisfied and \texttt{0x24} when the requested object or tag is absent; see Subsection~\ref{sec:new-piv-error-codes}. If the credential is not present or is removed during processing, PDs may return error code \texttt{0x27}.
\end{itemize}
...
\subsection{PIV Error Codes}\label{sec:new-piv-error-codes}

The following error codes are carried in \texttt{osdp\_PIVERRORR}. They extend the set defined in OSDP~2.2 without reassigning existing values and are available to any PIV/VCI command that requires them.
...
\begin{center}
  ...
  \begin{tabular}{|c|p{9cm}|}
    ...
    0x20 & Function not supported by credential (or PD does not implement the command). \\\hline
    0x21 & Insufficient PD memory or storage to complete the requested operation. \\\hline
    0x22 & Insufficient credential memory or storage space. \\\hline
    0x23 & Credential security status not satisfied. \\\hline
    0x24 & Data object or tag not found. \\\hline
    0x25 & VCI not established. \\\hline
    0x26 & PIV Auto not configured. \\\hline
    0x27 & Credential not present or removed. \\\hline
    0x28 & Biometric type not supported. \\\hline
  \end{tabular}
\end{center}
\end{samepage}

\subsubsection{PIV Application Error Reply (osdp\_PIVERRORR)}

PDs \textbf{SHALL} use \texttt{osdp\_PIVERRORR} to report application-level errors for PIV, VCI, and TWIC commands. Error responses \textbf{MAY} carry standard OSDP codes such as \texttt{0x03} (unsupported command), \texttt{0x05} (incorrect parameter), \texttt{0x06} (secure channel required), and \texttt{0x07} (multi-part sequencing failed) alongside the PIV-specific codes in Table~\ref{sec:new-piv-error-codes}. Transport-level faults (framing, CRC, sequence) continue to use \texttt{osdp\_NAK}. No additional detail fields are defined in this profile.

\input{tables/osdp-piverrorr}
