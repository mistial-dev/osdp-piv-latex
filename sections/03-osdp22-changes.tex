...
The reply continues to use the OSDP~2.2 multi-part format. Successful execution yields a response containing the credential’s entire \texttt{GENERAL AUTHENTICATE} data (minus the ISO/IEC~7816 status word), including the returned Dynamic Authentication Template; PDs \textbf{SHALL NOT} strip outer BER-TLV tags or lengths. If the credential reports an error, the PD \textbf{SHALL} return \texttt{osdp\_PDERROR} with the applicable PIV code—for example \texttt{0x0120} (function not supported by credential), \texttt{0x0121} (insufficient PD resources), \texttt{0x0122} (insufficient credential storage), \texttt{0x0123} (credential security status not satisfied), \texttt{0x0124} (data object/tag not found), \texttt{0x0125} (VCI not established), or \texttt{0x0127} (credential not present/removed). Generic parameter/security preconditions may continue to use \texttt{0x0005} (incorrect parameter) and \texttt{0x0006}/\texttt{0x0007} (PIN verification failed/timeout) where those codes remain canonical.
...
\begin{center}
  ...
  \begin{tabular}{|c|p{9cm}|}
    ...
    0x0120 & Function not supported by credential (or PD does not implement the command). \\\hline
    0x0121 & Insufficient PD memory or storage to complete the requested operation. \\\hline
    0x0122 & Insufficient credential memory or storage space. \\\hline
    0x0123 & Credential security status not satisfied. \\\hline
    0x0124 & Data object or tag not found. \\\hline
    0x0125 & VCI not established. \\\hline
    0x0126 & PIV Auto not configured. \\\hline
    0x0127 & Credential not present or removed. \\\hline
    0x0128 & Biometric type not supported. \\\hline
  \end{tabular}
\end{center}
\end{samepage}
...
PDs \textbf{SHALL} use \texttt{osdp\_PDERROR} to report application-level errors. The error code is two bytes, encoded least-significant byte first. When a card/APDU failure occurs (for example SC\_ERROR or a PIV APDU failure), the PD \textbf{SHALL} append the ISO/IEC~7816 status word (SW1\textbar SW2). Multiple errors \textbf{MAY} be reported by setting the error count and including additional code/SW pairs. Transport-level faults (framing, CRC, sequence) continue to use \texttt{osdp\_NAK}.
